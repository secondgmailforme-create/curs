// Скрипт для дашборда админа - статистика и последние заявки
async function getAdminDashboardStats() {
    const url = "/api/admin/escalated-tickets";  
    const urlRating = "/api/ratings/admin/stats";  
        ...
        if (ratingValue && ratingComment) {
            try {
                ...
                if (ratingResponse.ok) {
                    ...
                    if (data.global && data.global.average_rating) {
                        ratingValue.innerHTML = `${Number(data.global.average_rating).toFixed(1)} ⭐`;
                        ratingValue.style.color = data.global.average_rating >= 4 ? 'green' : (data.global.average_rating >= 3 ? 'orange' : 'red');
                        ratingComment.innerHTML = data.global.total_ratings ? `${data.global.total_ratings} всего оценок` : 'Нет оценок';
                    } else {
                ...
                console.warn('Failed to load admin rating stats:', ratingError);
                ratingValue.innerHTML = '—';
                ratingComment.innerHTML = 'Ошибка';
            }
        }

        // Получение всех заявок админа (эскалации от экспертов)
...
async function getAdminDashboardTickets(){
    const url = "/api/admin/escalated-tickets";  
    ...
    try{
            ...
            if(data.length > 3){
                ...
                button.innerHTML = `
                    <td colspan="6" style="text-align: center;">
                        <a href="/ADMIN-TICKETS" class="more-link" data-page="admin-tickets" data-i18n="allTicketsLink">Все заявки</a>
                    </td>
                `;
        ...
        console.error('AdminDashboard: Tickets error', error);
        const row = document.createElement('tr');
...
window.getAdminDashboardTickets = getAdminDashboardTickets;